...
""" Testing suite for the PyTorch OpenFlamingo model. """
...
from transformers import CONFIG_MAPPING, OpenFlamingoConfig, OpenFlamingoQFormerConfig, OpenFlamingoVisionConfig
...
if is_torch_available():
    ...
    from transformers import OpenFlamingoForConditionalGeneration, OpenFlamingoModel, OpenFlamingoVisionModel
    from transformers.models.openflamingo.modeling_openflamingo import OPENFLAMINGO_PRETRAINED_MODEL_ARCHIVE_LIST
...
class OpenFlamingoVisionModelTester:
    def __init__(
        self,
        parent,
        batch_size=12,
    ...
    def get_config(self):
        return OpenFlamingoVisionConfig(
    ...
    def create_and_check_model(self, config, pixel_values):
        model = OpenFlamingoVisionModel(config=config)
...
@require_torch
class OpenFlamingoVisionModelTest(ModelTesterMixin, unittest.TestCase):
    """
    Here we also overwrite some of the tests of test_modeling_common.py, as OpenFlamingo's vision encoder does not use input_ids, inputs_embeds,
    attention_mask and seq_length.
    """

    all_model_classes = (OpenFlamingoVisionModel,) if is_torch_available() else ()
    fx_compatible = False
    test_pruning = False
    ...
    def setUp(self):
        self.model_tester = OpenFlamingoVisionModelTester(self)
        self.config_tester = ConfigTester(
            self, config_class=OpenFlamingoVisionConfig, has_text_modality=False, hidden_size=37
        )

    def test_config(self):
        self.config_tester.run_common_tests()

    @unittest.skip(reason="OpenFlamingo's vision encoder does not use inputs_embeds")
    ...
    @unittest.skip(reason="OpenFlamingoVisionModel has no base class and is not available in MODEL_MAPPING")
    ...
    def test_model_from_pretrained(self):
        for model_name in OPENFLAMINGO_PRETRAINED_MODEL_ARCHIVE_LIST[:1]:
            model = OpenFlamingoVisionModel.from_pretrained(model_name)
            self.assertIsNotNone(model)


class OpenFlamingoQFormerModelTester:
    def __init__(
        self,
        parent,
        batch_size=12,
    ...
    def get_config(self):
        return OpenFlamingoQFormerConfig(
...
class OpenFlamingoTextModelDecoderOnlyTester:
    def __init__(
        self,
        parent,
        batch_size=12,
...
class OpenFlamingoForConditionalGenerationDecoderOnlyModelTester:
    def __init__(
        ...
        self.parent = parent
        self.vision_model_tester = OpenFlamingoVisionModelTester(parent, **vision_kwargs)
        self.qformer_model_tester = OpenFlamingoQFormerModelTester(parent, **qformer_kwargs)
        self.text_model_tester = OpenFlamingoTextModelDecoderOnlyTester(parent, **text_kwargs)
        self.is_training = is_training
    ...
    def get_config(self):
        return OpenFlamingoConfig.from_vision_qformer_text_configs(
    ...
    def create_and_check_for_conditional_generation(self, config, input_ids, attention_mask, pixel_values):
        model = OpenFlamingoForConditionalGeneration(config).to(torch_device).eval()
        with torch.no_grad():
...
@require_torch
class OpenFlamingoForConditionalGenerationDecoderOnlyTest(ModelTesterMixin, unittest.TestCase):
    all_model_classes = (OpenFlamingoForConditionalGeneration,) if is_torch_available() else ()
    fx_compatible = False
    test_head_masking = False
    test_pruning = False
    test_resize_embeddings = False
    ...
    def setUp(self):
        self.model_tester = OpenFlamingoForConditionalGenerationDecoderOnlyModelTester(self)
    ...
    @unittest.skip(reason="OpenFlamingoModel does not have input/output embeddings")
    def test_model_common_attributes(self):
        pass

    @unittest.skip(reason="There's no base OpenFlamingoModel")
    def test_save_load_fast_init_from_base(self):
        pass
    ...
    def test_load_vision_qformer_text_config(self):
        ...
        # Save OpenFlamingoConfig and check if we can load OpenFlamingoVisionConfig from it
        with tempfile.TemporaryDirectory() as tmp_dir_name:
            config.save_pretrained(tmp_dir_name)
            vision_config = OpenFlamingoVisionConfig.from_pretrained(tmp_dir_name)
            self.assertDictEqual(config.vision_config.to_dict(), vision_config.to_dict())

        # Save OpenFlamingoConfig and check if we can load OpenFlamingoQFormerConfig from it
        with tempfile.TemporaryDirectory() as tmp_dir_name:
            config.save_pretrained(tmp_dir_name)
            qformer_config = OpenFlamingoQFormerConfig.from_pretrained(tmp_dir_name)
            self.assertDictEqual(config.qformer_config.to_dict(), qformer_config.to_dict())

    @slow
    def test_model_from_pretrained(self):
        for model_name in OPENFLAMINGO_PRETRAINED_MODEL_ARCHIVE_LIST:
            model = OpenFlamingoForConditionalGeneration.from_pretrained(model_name)
            self.assertIsNotNone(model)


# this class is based on `T5ModelTester` found in tests/models/t5/test_modeling_t5.py
class OpenFlamingoTextModelTester:
    def __init__(
        self,
        parent,
        vocab_size=99,
...
class OpenFlamingoModelTester:
    def __init__(
        ...
        self.vision_model_tester = OpenFlamingoVisionModelTester(parent, **vision_kwargs)
        self.qformer_model_tester = OpenFlamingoQFormerModelTester(parent, **qformer_kwargs)
        self.text_model_tester = OpenFlamingoTextModelTester(parent, **text_kwargs)
        self.is_training = is_training
        self.num_query_tokens = num_query_tokens
...
@require_torch
class OpenFlamingoModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (OpenFlamingoForConditionalGeneration, OpenFlamingoModel) if is_torch_available() else ()
    fx_compatible = False
    test_head_masking = False
    test_pruning = False
    test_resize_embeddings = False
    ...
    def setUp(self):
        self.model_tester = OpenFlamingoModelTester(self)
    ...
    def test_get_text_features(self):
        ...
        model = OpenFlamingoModel(config).to(torch_device)
        model.eval()
...
@require_torch
@slow
class OpenFlamingoModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_opt_multi_gpu(self):
        processor = Blip2Processor.from_pretrained("vivym/openflamingo-9b")
        model = OpenFlamingoForConditionalGeneration.from_pretrained(
            "vivym/openflamingo-9b", torch_dtype=torch.float16, device_map="balanced"
        )
